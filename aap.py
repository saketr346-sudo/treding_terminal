...
# Live Header with Date and Time
st.markdown("### ⚡ Trading Terminal Dashboard")
st.markdown(f"🔴 <span style='color:#10b981; font-weight:bold;'>LIVE STREAM</span> | 📅 **{datetime.now().strftime('%d %b %Y')}** | 🕒 **{datetime.now().strftime('%I:%M:%S %p IST')}**", unsafe_allow_html=True)
    ...
    for pos in st.session_state.portfolio:
        ...
        if stock_info:
            ...
            if pos['target'] > 0 and curr_ltp >= pos['target']:
                returns = pos['capital_used'] + ((curr_ltp - pos['buy_price']) * pos['quantity'])
                st.session_state.virtual_balance += returns
                st.toast(f"🎯 TARGET HIT! Auto-Sold {pos['quantity']} shares of {pos['symbol']} at ₹{curr_ltp}", icon="🎉")
            
            elif pos['stop_loss'] > 0 and curr_ltp <= pos['stop_loss']:
                returns = pos['capital_used'] + ((curr_ltp - pos['buy_price']) * pos['quantity'])
                st.session_state.virtual_balance += max(0, returns)
                st.toast(f"🛑 STOPLOSS HIT! Auto-Sold {pos['quantity']} shares of {pos['symbol']} at ₹{curr_ltp}", icon="⚠️")
...
with st.container(border=True):
    ...
    with opt1:
        with st.form(key="top_search_form"):
            ...
            with c1:
                current_idx = st.session_state.ticker_list.index(st.session_state.selected_symbol) if st.session_state.selected_symbol in st.session_state.ticker_list else 0
                dropdown_stock = st.selectbox("Select Stock", options=st.session_state.ticker_list, index=current_idx)
            with c2:
                custom_stock = st.text_input("Or Type Ticker", value="", placeholder="e.g. SUZLON")
            with c3:
                selected_tf = st.selectbox("Timeframe", ["1M", "3M", "6M", "1Y", "3Y", "5Y", "10Y"], index=0)
            with c4:
                st.write("")
                btn_search = st.form_submit_button(label="🔍 Search Stock", use_container_width=True)

        if btn_search:
            if custom_stock.strip():
                raw_symbol = custom_stock.strip().upper()
                formatted_symbol = f"{raw_symbol}.NS" if not (raw_symbol.endswith(".NS") or raw_symbol.endswith(".BO")) else raw_symbol
    ...
    with opt3:
        ...
        with w_c1:
            ...
            for idx, w_stock in enumerate(st.session_state.watchlist):
                if wl_cols[idx % 4].button(f"📈 {w_stock}", key=f"top_wl_{w_stock}", use_container_width=True):
                    st.session_state.selected_symbol = w_stock
                    st.rerun()
        with w_c2:
            add_wl = st.text_input("Add Stock to Watchlist", placeholder="e.g. ZOMATO", key="add_wl_top")
    ...
    with opt4:
        st.markdown(f"💰 Available Cash Balance: **₹{st.session_state.virtual_balance:,.2f}**")
        v_left, v_right = st.columns([1, 1])
        
        with v_left:
            st.markdown("##### 📥 Order Execution Panel")
            v_stock = st.selectbox("Stock to Trade", options=st.session_state.ticker_list, index=st.session_state.ticker_list.index(st.session_state.selected_symbol) if st.session_state.selected_symbol in st.session_state.ticker_list else 0)
            
            c_q, c_m = st.columns(2)
            with c_q:
                v_qty = st.number_input("Quantity", min_value=1, value=10, step=1)
            with c_m:
                v_margin = st.selectbox("Margin", ["1x (Delivery)", "2x (Intraday)", "5x (Super Margin)"], index=0)
            ...
            st.caption(f"Required Margin: **₹{req_amount:,.2f}** | Current LTP: **₹{v_ltp}**")
            ...
            with btn_sell_col:
                if st.button(f"🔴 SELL {v_stock}", use_container_width=True):
                    holdings = [p for p in st.session_state.portfolio if p['symbol'] == v_stock]
                    if holdings:
                        recovered_cash = sum([p['capital_used'] + ((v_ltp - p['buy_price']) * p['quantity']) for p in holdings])
                        st.session_state.virtual_balance += recovered_cash
                        st.session_state.portfolio = [p for p in st.session_state.portfolio if p['symbol'] != v_stock]
        ...
        with v_right:
            ...
            if st.session_state.portfolio:
                tot_invested = sum([p['capital_used'] for p in st.session_state.portfolio])
                ...
                for idx, pos in enumerate(st.session_state.portfolio):
                    ...
                    with st.container(border=True):
                        ...
                        with col_p1:
                            st.markdown(f"**{pos['symbol']}** | Qty: {pos['quantity']} | {pos['margin']}")
                            st.caption(f"Buy: ₹{pos['buy_price']} | SL: ₹{pos['stop_loss']} | Target: ₹{pos['target']}")
                        with col_p2:
                            st.markdown(f"<div style='text-align:right;'>LTP: <b>₹{curr_p}</b><br><span style='color:{pnl_color}; font-weight:bold;'>₹{pnl:,.2f}</span></div>", unsafe_allow_html=True)
            else:
...
if data:
    ...
    pct_val = round(((ltp - prev_close) / prev_close) * 100, 2) if prev_close != 0 else 0.0
    ...
    sample_news = [
        {"title": f"{symbol} price momentum tracking at {pct_val:+.2f}%.", "tag": "BULLISH" if pct_val >= 0 else "BEARISH", "score": dynamic_sentiment_score},
    ...
    with m2:
        with st.container(border=True):
            st.metric(label="Change", value=f"₹{change_val:+}", delta=f"{pct_val:+.2f}%")
    with m3:
        with st.container(border=True):
    ...
    with chart_col:
        with st.container(border=True):
            ...
            fig = make_subplots(rows=2, cols=1, shared_xaxes=True, vertical_spacing=0.02, row_heights=[0.80, 0.20])

            fig.add_trace(go.Candlestick(
                x=hist.index, open=hist['Open'], high=hist['High'], low=hist['Low'], close=hist['Close'],
                name="OHLC", increasing_line_color='#00D09C', increasing_fillcolor='#00D09C',
                decreasing_line_color='#FF5353', decreasing_fillcolor='#FF5353'
            ), row=1, col=1)

            fig.add_trace(go.Scatter(x=hist.index, y=hist['MA20'], mode='lines', name='20 MA', line=dict(color='#00B8D9', width=1.8)), row=1, col=1)
            fig.add_trace(go.Scatter(x=hist.index, y=hist['Upper_Band'], mode='lines', name='Upper Band', line=dict(color='rgba(0, 208, 156, 0.3)', width=1, dash='dash')), row=1, col=1)
            fig.add_trace(go.Scatter(x=hist.index, y=hist['Lower_Band'], mode='lines', name='Lower Band', line=dict(color='rgba(255, 83, 83, 0.3)', width=1, dash='dash'), fill='tonexty', fillcolor='rgba(255, 255, 255, 0.015)'), row=1, col=1)

            volume_colors = ['#00D09C' if c >= o else '#FF5353' for c, o in zip(hist['Close'], hist['Open'])]
            fig.add_trace(go.Bar(x=hist.index, y=hist['Volume'], name='Volume', marker_color=volume_colors, opacity=0.6), row=2, col=1)

            fig.update_layout(
                height=480, margin=dict(l=10, r=10, t=25, b=10), xaxis_rangeslider_visible=False,
                paper_bgcolor='#121418', plot_bgcolor='#121418', font=dict(color='#8C96A3'), hovermode="x unified"
            )
